...
def load_nucleus_and_periphery_from_files(target_nucleus_file, target_periphery_file, do_plotting=False):
    ...
    nucleus_contour = measure.find_contours(nucleus_image[:,:], 100)[0]
    nucleus_center = center_of_mass(nucleus_image[:,:])
    nucleus_contour_shape = LinearRing(nucleus_contour)
    periphery_image = skimage.io.imread(target_periphery_file)
    periphery_contours = measure.find_contours(periphery_image[:,:], 100)
...
def test_perinuclearity(target_nucleus_file, target_periphery_file, target_dic_file, target_labels_file=False):
    ...
    if not target_labels_file:
        ...
        labels_image = tiff.imread(target_labels_file)
...
def plot_perinuclearity_isolevels():
    ...
    target_nucleus_file = 'data/WT/1_WT black nucleous.tif'
    target_periphery_file = 'data/WT/1_WT black outside.tif'
    nucleus_center, nucleus_contour_shape, periphery_contour_shape = \
        load_nucleus_and_periphery_from_files(target_nucleus_file, target_periphery_file,
                                              do_plotting=False)
